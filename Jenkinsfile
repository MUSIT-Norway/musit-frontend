...
node {
    ...
    stage('Test') {
        echo "Run tests for branch ${gitBranch}"
    ...
    if (gitBranch == "master" || gitBranch == "jenkinstest") {
        stage('Build') {
            ...
            sh "docker tag harbor.uio.no:443/musit/webpack harbor.uio.no:443/musit/webpack:${gitCommit}"
            sh "docker tag harbor.uio.no:443/musit/webpack harbor.uio.no:443/musit/webpack:utv"
        }
        
        if (gitBranch == "master" || (gitBranch == "jenkinstest" && deployJenkinstest)) {
            ...
            stage('Publish to harbor') {
                ...
                // sh "docker push harbor.uio.no:443/musit/webpack:${gitCommit}"
            }
        }
        ...
        stage('Mirror to GitHub') {
            withCredentials([string(credentialsId: 'GitHubMirrorToken', variable: 'GIT_HUB_MIRROR_TOKEN')]) {
                echo "push branch ${gitBranch} to github as branch ${gitBranch}2"
                sh "git push https://git:${GIT_HUB_MIRROR_TOKEN}@github.com/MUSIT-Norway/musit-frontend.git HEAD:${gitBranch}2"
            }
       }
    }
}
