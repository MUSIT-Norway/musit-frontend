export const LOAD_SEVERAL = 'musit/storageunit-grid/LOAD_SEVERAL';
export const LOAD_SEVERAL_SUCCESS = 'musit/storageunit-grid/LOAD_SEVERAL_SUCCESS';
export const LOAD_SEVERAL_FAIL = 'musit/storageunit-grid/LOAD_SEVERAL_FAIL';
export const LOAD_ONE = 'musit/storageunit-grid/LOAD_ONE';
export const LOAD_ONE_SUCCESS = 'musit/storageunit-grid/LOAD_ONE_SUCCESS';
export const LOAD_ONE_FAIL = 'musit/storageunit-grid/LOAD_ONE_FAIL';
export const CLEAR_ROOT = 'musit/storageunit-grid/CLEAR_ROOT';
export const DELETE = 'musit/storageunit-grid/DELETE';
export const DELETE_SUCCESS = 'musit/storageunit-grid/DELETE_SUCCESS';
export const DELETE_FAIL = 'musit/storageunit-grid/DELETE_FAIL';

export const LOAD_OBJECTS = 'musit/storageobject-grid/LOAD_SEVERAL';
export const LOAD_OBJECTS_SUCCESS = 'musit/storageobject-grid/LOAD_SEVERAL_SUCCESS';
export const LOAD_OBJECTS_FAIL = 'musit/storageobject-grid/LOAD_SEVERAL_FAIL';

export const LOAD_STATS = 'musit/storageunit-stats/LOAD_STATS';
export const LOAD_STATS_SUCCESS = 'musit/storageunit-stats/LOAD_STATS_SUCCESS';
export const LOAD_STATS_FAILURE = 'musit/storageunit-stats/LOAD_STATS_FAILURE';
export const CLEAR_STATS = 'musit/storageunit-stats/CLEAR_STATS';

export const LOAD_MAIN_OBJECT = 'musit/picklist/LOAD_MAIN_OBJECT';
export const LOAD_MAIN_OBJECT_SUCCESS = 'musit/picklist/LOAD_MAIN_OBJECT_SUCCESS';
export const LOAD_MAIN_OBJECT_FAIL = 'musit/picklist/LOAD_MAIN_OBJECT_FAIL';