...
export const loadAnalysis$ = createAction('loadAnalysis$').switchMap(Analysis.getAnalysisWithDeatils());
...
export const reducer$ = (actions) => Observable.merge(
  actions.loadAnalysisTypes$.map((analysisTypes) => (state) => ({...state, analysisTypes})),
  actions.loadAnalysis$.map((data) => (state) => ({...state, analysis: data, loading: false})),
  actions.getAnalysisTypesForCollection$.map((analysisTypes) => (state) => ({...state, analysisTypes }))
);

export const store$ = (actions$ = {getAnalysisTypesForCollection$, loadAnalysisTypes$, loadAnalysis$}) =>
  createStore('analysisStore', reducer$(actions$), Observable.of({ objectsData, analysisTypes: [], analysis: [] }));
