...
import {
  ...
  refreshNode, refreshObject
} from '../picklistActions';
import picklistReducer from '../picklistReducers';
import * as types from '../picklistTypes';
import MusitObject from '../../../models/object';
import reducer from '../picklistReducers';
...
describe('PicklistReducer', () => {
  ...
  it('Refresh node updates path. Do not show root if only root is there.', () => {
    const newState = picklistReducer(testState, {
      type: types.LOAD_ONE_NODE_SUCCESS,
      result:{
        path: ',1,',
        pathNames: [{ nodeId: 1}]
      },
  ...
  it('invokes LOAD_ONE_OBJECT_SUCCESS when object path is loaded.', () => {
    ...
    return store.dispatch(refreshObject(1, new MuseumId(99)))
        .then(() => {
          expect(store.getActions()).toMatchSnapshot();
        });
  });
  ...
  it('Refresh object: success action', () => {
    expect(
        reducer(undefined, {
          type: types.LOAD_ONE_OBJECT_SUCCESS,
          result: {
            someField: 1
          }
        })
  ...
  it('Refresh object: fail action', () => {
    expect(
        reducer(undefined, {
          type: types.LOAD_ONE_OBJECT_FAIL,
  ...
  it('invokes LOAD_ONE_NODE_SUCCESS when node path is loaded.', () => {
    ...
    return store.dispatch(refreshNode(3, new MuseumId(99)))
        .then(() => {
          expect(store.getActions()).toMatchSnapshot();
        });
  });
